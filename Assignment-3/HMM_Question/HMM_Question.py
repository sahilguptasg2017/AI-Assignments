...
def emission_probability(state, observation,sigma):
    ...
    currcoor = state[0]
    eucliddistsq = ((currcoor[0]-observation[0])**2) + ((currcoor[1]-observation[1])**2)
    return -0.5*(eucliddistsq/sigma**2) + np.log(1/(sigma*np.sqrt(2*np.pi)))
...
def transition_probability(prev_state, curr_state, movement_policy):
    ...
    """
    ###### YOUR CODE HERE ######
    prevcoor , prevhead = prev_state ; prevx , prevy = prevcoor
    currcoor , currhead = curr_state ; currx , curry = currcoor
    changex = currx - prevx ; changey = curry - prevy
    if movement_policy == 'straight_until_obstacle': 
        if (prevx == 0 and prevhead == 'W') or (prevx == GRID_WIDTH-1 and prevhead == 'E') or (prevy == 0 and prevhead == 'N') or (prevy == GRID_HEIGHT-1 and prevhead == 'S'):
            posstates = [(prevx - 1 , prevy) , (prevx + 1 , prevy) , (prevx , prevy - 1) , (prevx , prevy + 1)]
            truestates = []
            # cnt = 0
            for x in posstates:
                if not is_obstacle(x):
                    truestates.append(x)
                    # cnt += 1
                else:
                    truestates.append(prevcoor)
            cnt = 0
            for x in truestates:
                if x == currcoor:
                    cnt += 1
        ...
        elif prevhead == 'N' and changex == 0 and changey == -1 and currhead == 'N':
            return 0.0
        elif prevhead == 'E' and changex == 1 and changey == 0 and currhead == 'E':
            return 0.0
        elif prevhead == 'S' and changex == 0 and changey == 1 and currhead == 'S':
            return 0.0
        elif prevhead == 'W' and changex == -1 and changey == 0 and currhead == 'W':
            return 0.0
        else:
            return float('-inf')
...
def viterbi(observations, start_state, movement_policy,states,sigma):
    ...
    for i in range(len(states)):
        emissionProb = emission_probability(states[i],observations[0],sigma)
        if states[i]!=start_state:
            storeTimes[0][states[i]] = float('-inf')
        else:
            storeTimes[0][states[i]] = emissionProb
            ans[states[i]] = [states[i]]
    for i in range(1,len(observations)):
        ...
        for curr in states:
            ...
            prevstate = None
            for prev in states:
                prob = storeTimes[i-1][prev] + transition_probability(prev , curr , movement_policy) + emission_probability(curr,observations[i],sigma)
                if prob > maxprob:
                    maxprob = prob
                    prevstate = prev
            storeTimes[i][curr] = maxprob
            if prevstate!=None:
                subans[curr] = ans[prevstate] + [curr]
        ans = subans
    maxprob = float('-inf')
    laststate = None
    for st , prob in storeTimes[-1].items():
        if prob > maxprob :
            maxprob = prob
            laststate = st
    if laststate != None:
        return ans[laststate]
    else:
        return []
...
if __name__ == "__main__":
    ...
    for policy in policies:
        ...
        with open('estimated_paths.csv', 'a', newline='') as file:
            file.write(f'{seed},{policy},"{estimated_path}"\n')
